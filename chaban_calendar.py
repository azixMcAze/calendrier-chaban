...
def day_filter_predicate(bridge_event: BridgeEvent, day_filter: list[int]):
    return bridge_event.closingTime.weekday() in day_filter


def filter_by_day(bridge_data: Iterable[BridgeEvent], day_filter: Optional[list[int]]):
...
def convert_json_to_cal(json_text: str, day_filter: Optional[list[int]] = None) -> bytes:
